...
class rabbitmq::install::rabbitmqadmin {
  ...
  $management_bind_ip_address = $rabbitmq::management_bind_ip_address

  staging::file { 'rabbitmqadmin':
    target      => "${rabbitmq::rabbitmq_home}/rabbitmqadmin",
    source      => "${protocol}://${default_user}:${default_pass}@${management_bind_ip_address}:${management_port}/cli/rabbitmqadmin",
